...
这样做可以避免跟现有的以及未来的 HTML 元素?<p style="color: #f00;">相冲突</p>，因为所有的 HTML 元素名称都是单个单词的。
...
在组件上_总是_必须用 `key` 配合 `v-for`，以便维护内部组件及其子树的状态。甚至在元素上维护可预测的行为，比如动画中的<p style="color: #0f0;"> [对象固化(object constancy)](https://bost.ocks.org/mike/constancy/) </p>，也是一种好的做法。
...
这里的问题在于，不要删除仍然会留在 DOM 中的元素。比如你想使用 <span style="color: #f00;">```<transition-group>```</span> 给列表加过渡动画，或想在被渲染元素是  <span style="color: #f00;">```<input />```</span> 时保持聚焦。在这些情况下，为每一个项目添加一个唯一的键值 (比如 :key="todo.id") 将会让 Vue 知道如何使行为更容易预测。
...
### 参考文档
[v2/style-guide](https://v2.cn.vuejs.org/v2/style-guide/)
